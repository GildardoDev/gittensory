...
test("pypiHealth: a malformed QUERIED release fails safe even when another release looks old (no false stale)", () => {
  // The gate's exact regression case: 1.0.0 resolves but is malformed ({}), 0.9.0 is old — 1.0.0 must NOT be
  // reported stale from another release's date.
  assert.equal(
    pypiHealth(
      { releases: { "1.0.0": {}, "0.9.0": [{ upload_time_iso_8601: iso(NOW - 4 * YEAR_MS) }] } },
      "1.0.0",
      STALE_MS,
      NOW,
    ),
  ...
  // empty file list and mixed malformed-plus-valid file arrays also fail safe.
  assert.equal(pypiHealth({ releases: { "1.0.0": [] } }, "1.0.0", STALE_MS, NOW), null);
  assert.equal(
    pypiHealth({ releases: { "1.0.0": [null, { upload_time_iso_8601: iso(NOW) }] } }, "1.0.0", STALE_MS, NOW),
    null,
  );
});

test("npmHealth: a queried version absent from the packument's versions map yields no finding", () => {
  // versions enumerates 0.9.0 only; querying 1.0.0 (which would otherwise look stale from `time`) must not flag.
  assert.equal(
    npmHealth(
      { versions: { "0.9.0": {} }, time: { "0.9.0": iso(NOW - 4 * YEAR_MS) } },
      "1.0.0",
      STALE_MS,
      NOW,
    ),
...
test("scanDepMaintenanceHealth: end-to-end stale finding via the injected now clock", async () => {
  const findings = await scanDepMaintenanceHealth(
    npmAdd("oldpkg"),
    npmFetch({ time: { "1.0.0": iso(NOW - 4 * YEAR_MS) } }),
    { now: NOW },
  );
  assert.equal(findings.length, 1);
...
test("renderBrief emits a public-safe deprecated/stale block and escapes registry-supplied reasons", () => {
  ...
  assert.match(promptSection, /Deprecated \/ yanked \/ stale dependencies/);
